...
class MHS(nn.Module):
    ...
    def __init__(self, dispersed_data=True):
        super().__init__()
        self.dispersed_data = dispersed_data

        self.lst0 = HSBlock(256)
        self.lst1 = HSBlock(256)
        self.lst2 = HSBlock(256)
        self.lst3 = HSBlock(256)

    def forward(self, in_feats: List[torch.Tensor]):
        return (self.lst0(in_feats[0]),
                self.lst1(in_feats[1]),
                self.lst2(in_feats[2]),
                self.lst1(in_feats[3]))


class DINO_H_Res_mixFPN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        multi_level_feats = self.MHS(multi_level_feats)
        multi_level_masks = []
